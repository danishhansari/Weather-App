  ...
  fetchData(url.currentWeather(lat, lon), function (currentWeather) {
    ...
    fetchData(url.forecast(lat, lon), function (forecast) {
      ...
      for (const [index, data] of forecastList.entries()) {
        ...
        <div class="card card-sm slider-card">
        <p class="body-3">${module.getHours(dateTimeUnix, timezone)}</p>
        <img src="./assets/images/weather_icons/direction.png" width="48" height="48" loading="lazy" class="weather-icon" alt="direction" style="transform:rotate(${windDirection - 180}deg)">
    ...
    `;
    for( let i = 7 , len = forecast.length; i<len; i+=8){
         const {
            main: {temp_max},
            weather,
            dt_txt
